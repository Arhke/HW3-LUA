...
for i in range(10):
    ...
    p = Popen(["dotnet", "run", "-c", "Release", "--project", project, "sectype", "type-leak.mc"], stdout=PIPE, stdin=PIPE, stderr=PIPE)
    stdout_data = p.communicate(input=str(random_number).encode())[0].decode()
    print("Program output: ", stdout_data)

    match = re.search("Program complete in (\d+) ms", stdout_data)
