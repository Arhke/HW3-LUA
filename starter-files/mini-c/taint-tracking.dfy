...
function EvalExprTaint(d:Declarations, s:TaintState, e:Expr, t:Type) : (tv:TaintValue)
...
function UpdateVarTaint(s:TaintState, v:Variable, taint:bool) : TaintState {
    s.(var_map := s.var_map[v := taint])
}

// Update the store within taint_state, so that v now maps to val
function UpdateVarVal(s:TaintState, v:Variable, val:Value) : TaintState {
    s.(store := s.store[v := val])
}

// Update the taint setting for the PC within taint_state
function UpdatePCTaint(s:TaintState, taint:bool) : TaintState {
    s.(pc_tainted := taint)
}

function {:extern} PrintErr(s:string) : ()

//
// Our taint-tracking engine
//
function EvalCommandTaint(d:Declarations, s:TaintState, c:Command) : (t:TResult)
...
lemma TaintExamplesCommand(io: IO) {
    ...
    taintMap := map[x := true, y := false];
    taintState := TaintState(1, store, io, false, taintMap);
    assert ExprHasType(d, Int(1), TInt);
    assert ExprHasType(d, cond, TBool);
    assert EvalExprTaint(d, taintState, cond, TBool).tainted;
    assert CommandWellTyped(d, ifFalse);
