...
function EvalExpr(e:Expr, store:Store) : EResult {
    match e        
...
// as documented by the "{:axiom}" annotation; it's up to 
// the developer of that implementation to make sure that 
// assumption holds.
function {:extern} {:axiom} PrintString(s:string, ghost io:IO) : (io':IO)
...
function {:extern} {:axiom} ReadInt(ghost io:IO) : (int, IO)
    ensures 
        // The io that is returned is the same as before,
...
function {:extern} {:axiom} ReadSecretInt(ghost io:IO) : (int, IO)
    ensures 
        // Same as for ReadInt
...
function Bool2String(b:bool) : string {
    if b then "true" else "false"
}
function {:extern} Int2String(i:int) : string
...
function EvalCommand(s:State, c:Command) : CResult 
    decreases s.fuel, c
{
...
predicate ExprHasType(d:Declarations, e:Expr, t:Type) {
    match e
...
predicate CommandWellTyped(d:Declarations, c:Command) {
    match c
        case Noop => true
