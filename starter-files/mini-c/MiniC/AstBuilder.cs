...
namespace _module
{
  internal class AstBuilder : MiniCBaseVisitor<object>
  {
    ...
    private ISequence<Dafny.Rune> MakeRunes(string s) {
      return Sequence<Dafny.Rune>.UnicodeFromString(s);
    }

    private ISequence<Dafny.Rune> MakeRunesFromId(MiniCParser.IdContext id) {
      return MakeRunes(id.name.Text);
    }
    private Variable MakeVariable(MiniCParser.IdContext id) {
      //return new Variable(Sequence<Dafny.Rune>.UnicodeFromString(id.name.Text));
      return new Variable(MakeRunesFromId(id));
    }
    ...
    public override object VisitExp(MiniCParser.ExpContext context) {
      if (context.b != null) {
        ...
        return Expr.create_Var(MakeRunesFromId(context.v));
    ...
    public override object VisitAssignCommand(MiniCParser.AssignCommandContext context) {
      return Command.create_Assign(MakeRunesFromId(context.v), (Expr) VisitExp(context.e));
    }
    ...
    public override object VisitPrintSCommand(MiniCParser.PrintSCommandContext context) {
      return Command.create_PrintS(MakeRunes(context.s.Text.Trim('\'')));
    }
    ...
    public override object VisitGetIntCommand(MiniCParser.GetIntCommandContext context) {
      return Command.create_GetInt(MakeRunes(context.v.name.Text));
    }

    public override object VisitGetSecretIntCommand(MiniCParser.GetSecretIntCommandContext context) {
      return Command.create_GetSecretInt(MakeRunes(context.v.name.Text));
    }
  }
